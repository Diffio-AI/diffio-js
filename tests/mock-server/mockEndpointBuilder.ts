import { HttpResponse, http, type DefaultBodyType, type HttpHandler, type HttpResponseResolver } from "msw";
...
class RequestBuilder implements MethodStage, RequestHeadersStage, RequestBodyStage, ResponseStage {
  ...
  private buildUrl(): string {
    return join(this.baseUrlValue, this.path);
  }
}
...
class ResponseBuilder implements ResponseStatusStage, ResponseHeaderStage, ResponseBodyStage, BuildStage {
  ...
  statusCode(code: number): ResponseHeaderStage {
  ...
  build(): HttpHandler {
    const responseResolver: HttpResponseResolver = () => {
      ...
      if (!Object.keys(this.responseHeaders).some((key) => key.toLowerCase() === "content-type")) {
