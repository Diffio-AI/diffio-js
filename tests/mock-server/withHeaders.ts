...
export function withHeaders(
  ...
  return (args) => {
    ...
    const mismatches: Record<string, { actual: string | null; expected: string | RegExp | ((value: string) => boolean) }> =
      {};
    ...
    if (Object.keys(mismatches).length > 0) {
      console.error("Header mismatch:", formatHeaderMismatches(mismatches));
      return passthrough();
    }
